...
def read_img():
    '''
    Lê as imagens das letras 'a, b, c, d, e, f, i, o, u' contidas
    na pasta 'letras'. 
    ...
    x_treino, x_teste, y_treino, y_teste = sk.train_test_split(x,y, test_size=0.20, random_state=42)
    return x_treino, x_teste, y_treino, y_teste


def pre_processamento(x_treino, x_teste, ruidos='g'):
    '''
    Primeiro escala os dados que originalmente estão entre [0, 255] para o intervalo [0, 1].
...
def plt1():
    '''
    Print das N figuras sem ruído (linha 1) e com ruído (linha 2)
    '''
...
def plt2():
    '''
    Print de N imagens sem ruído (linha 1), com ruído (linha 2), 
    codificada (linha 3) e decodificada (linha 4)
    '''
    n = 10  #  Quantidade de letras impressas
...
def SAE_Model():
    ...
    input_dim = x_treino.shape[1] #  tamanho da camada de entrada (qtd de pixels da imagem)
    
    # Rede Neural SAE
    autoencoder = Sequential()
    
    # Encoder Layers
    autoencoder.add(Dense(4 * encoding_dim, input_shape=(input_dim,), activation='relu')) # 625 -> 100
...
def SAE_2D():
    ...
    # Encoder Layers
    autoencoder2D.add(Dense(64 * encoding_dim, input_shape=(input_dim,), activation='relu')) # 625 -> 100
    ...
    autoencoder2D.compile(optimizer='adadelta', loss='binary_crossentropy', metrics=['accuracy'])
    monitor = callbacks.EarlyStopping(monitor='loss', min_delta=1e-5, patience=2, verbose=2, mode='auto')
    # Treina a rede SAE
    SAE2D = autoencoder2D.fit(x_train_noisy, x_treino, epochs=70, batch_size=20, validation_data=(x_test_noisy, x_teste), callbacks=[monitor], verbose=2)
...
def plt2D(data, label, title):
    '''
    Plota a imagem codificada em 2D no plano cartesiano.
...
def features(res, label, title):   
    ...
    Essa função extrai a estatística dos dados plotados com a função plt2D().
...
def unzip(x, frame, letra):
    '''
    x: inteiro ou float que dá uma posição na reta real.
...
# START = Realiza procedimento completo, desde ler imagens, treinar SAE, treinar rede neural de avaliação e mostrar resultados
# BOOT = Carrega variáveis com valores salvos previamente, caso existam
MODE = 'START'
_2D = True
...
if MODE == 'START':
    ...
    autoencoder.compile( metrics=['accuracy']) #optimizer='adadelta',loss='binary_crossentropy',
    #monitor = callbacks.EarlyStopping(monitor='loss', min_delta=1e-5, patience=2, verbose=2, mode='auto')
    # Treina a rede SAE
    SAE = autoencoder.fit(x_train_noisy, x_treino, epochs=50, validation_data=(x_test_noisy, x_teste), verbose=2) #callbacks=[monitor], 
    
    # Valor da função custo e precisão da rede durante o treinamento
    #plt.plot(SAE.history['loss'])
    plt.plot(SAE.history['accuracy'])
    plt.legend(('Custo','Precisão'), loc='upper right')
    plt.title('Históricos')
    plt.xlabel('Época'); 
    plt.show()
    ...
    # Parâmetros para o treinamento da rede neural
    model.compile(metrics=['accuracy']) #optimizer='adadelta', loss='sparse_categorical_crossentropy'
    
    #monitor = callbacks.EarlyStopping(monitor='loss', min_delta=1e-3, patience=2, verbose=2, mode='auto')
    
    # Treina a rede MLP
    H = model.fit(x_treino, y_treino, epochs=40, validation_data=(x_teste, y_teste), verbose=2) #callbacks=[monitor],
    ...
    if _2D:
        ...
        res_x_test_noisy = plt2D(x_test_noisy, y_teste, x_test_noisy2D_title)
        
        x_treino2D_title = 'Representação em 2D dos DADOS DE TREINO (imagens originais)'
        res_x_train_noisy = plt2D(x_train_noisy, y_treino, x_treino2D_title)
